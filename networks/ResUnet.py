...
class UNetRes(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose', bias=True):
        super(UNetRes, self).__init__()

        self.m_head = B.conv(in_channels, nc[0], bias=bias, mode='C')
        ...
        self.m_tail = B.conv(nc[0], out_channels, bias=bias, mode='C')
...
#        x = nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))(x)
        ...
        permut = False
        if x0.shape[-1] == 2:
            x0 = torch.permute(x0, (0, 3, 1, 2))  # (B, C, H, W)
            permut = True
